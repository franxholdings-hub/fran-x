...
--
-- This migration adds the Digital Store ecosystem on top of the
-- existing FRAN-X schema. It does NOT modify existing tables
-- (ai_packages, subscriptions, payments, revenue_history, inquiries)
-- beyond adding optional reference columns. All new tables are
-- admin-managed (RLS) with public read for published/active rows.
--
-- Prices seeded here are the initial recommended values from the
-- FRAN-X specification and are fully editable from the admin
-- dashboard without source-code changes.
-- ============================================================

-- ------------------------------------------------------------
-- 1. DIGITAL PRODUCTS (templates, e-books, financial guides, bundles)
-- ------------------------------------------------------------
create table if not exists public.digital_products (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  name text not null,
  category text not null,                       -- templates | ebooks | finance
  price numeric not null default 0,
  currency text not null default 'NGN',
  description text not null default '',
  whats_included jsonb not null default '[]'::jsonb,
  file_format text not null default '',
  cover text,                                   -- photo key or URL
  featured boolean not null default false,
  is_bundle boolean not null default false,
  bundle_slugs jsonb not null default '[]'::jsonb,
  related_slugs jsonb not null default '[]'::jsonb,
  has_file boolean not null default false,      -- real file uploaded before publish
  file_url text,                                -- access-controlled storage path
...
-- ------------------------------------------------------------
-- 2. DIGITAL SERVICES (website, marketing, branding, automation, retainers)
-- ------------------------------------------------------------
create table if not exists public.digital_services (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  name text not null,
  group_label text not null,                     -- Website Services | Digital Marketing | ...
  service_group text not null,                   -- website | marketing | branding | automation | retainer
  price_from numeric not null default 0,
  billing_type text not null default 'one_time', -- one_time | monthly | annual | custom
...
-- ------------------------------------------------------------
-- 3. DIGITAL SUBSCRIPTION PLANS (Resource Pass + FRIX AI)
--    Separate from ai_packages to keep the Digital Store
--    subscription system independent, as required.
-- ------------------------------------------------------------
create table if not exists public.digital_plans (
  id uuid primary key default gen_random_uuid(),
  code text not null unique,
  name text not null,
  plan_type text not null,                       -- resource_pass | frix_ai
...
-- ------------------------------------------------------------
-- 4. DIGITAL LIBRARY — owned purchases vs subscription access.
--    One-time purchases persist; subscription access is revoked
--    when the subscription expires.
-- ------------------------------------------------------------
create table if not exists public.digital_library (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  product_slug text,                             -- owned one-time product
  plan_code text,                                 -- subscription plan code (access)
  access_type text not null default 'owned',     -- owned | subscription
  payment_id uuid references public.payments(id) on delete set null,
  subscription_id uuid references public.subscriptions(id) on delete set null,
  granted_at timestamptz not null default now(),
  expires_at timestamptz,                         -- null for owned (permanent)
  is_active boolean not null default true,
  created_at timestamptz not null default now()
);
...
-- ------------------------------------------------------------
-- 5. SERVICE REQUESTS — extends inquiries for digital services.
--    Reuses the inquiries table (kind = 'service'); this table
--    adds delivery tracking for the service dashboard.
-- ------------------------------------------------------------
create table if not exists public.service_requests (
  ...
  status text not null default 'Requested',      -- Requested | Reviewing | In Progress | Awaiting Customer | Completed
  assigned_to text,
  expected_delivery date,
  budget text,
  timeline text,
  goals text,
  payment_status text not null default 'unpaid',  -- unpaid | paid | custom_quote
  amount numeric,
...
-- ------------------------------------------------------------
-- 6. PROMOTIONS — discount codes, bundles, limited-time offers.
-- ------------------------------------------------------------
create table if not exists public.promotions (
  id uuid primary key default gen_random_uuid(),
  code text not null unique,                      -- e.g. STARTUP20
  description text,
  discount_type text not null default 'percentage', -- percentage | fixed
  discount_value numeric not null default 0,
  applies_to text not null default 'all',         -- all | product | service | subscription | product_slug
  target_slug text,
  max_uses int,
...
-- ============================================================
-- SEED DATA — initial recommended products, services and plans.
-- Prices here are editable from the admin dashboard.
-- ============================================================

-- Digital products (templates, e-books, finance, bundles)
...
-- Example promotion (not auto-applied)
